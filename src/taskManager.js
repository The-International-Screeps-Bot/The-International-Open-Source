function taskManger(room, myCreeps) {
    ...
    if (haulers.length > 0) {
        ...
        if (findCreepWithoutTask(haulers) && controllerLink == false && controllerContainer == true && ((storage && storage.store[RESOURCE_ENERGY] >= 30000) || (terminal && terminal.store[RESOURCE_ENERGY] >= 30000)) && controllerContainer.store[RESOURCE_ENERGY] <= findCreepWithoutTask(haulers).store.getCapacity()) {
