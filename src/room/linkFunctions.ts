...
Room.prototype.sourcesToReceivers = function (sourceLinks, receiverLinks) {
    ...
    for (const sourceLink of sourceLinks) {
        ...
        if (
            sourceLink.store.getCapacity(RESOURCE_ENERGY) -
        ...
        for (const receiverLink of receiverLinks) {
            ...
            if (
                ...
                receiverLink.store.getCapacity(RESOURCE_ENERGY) * 0.25
            )
                continue
            ...
            sourceLink.store.energy -=
                receiverLink.store.getCapacity(RESOURCE_ENERGY) -
                receiverLink.store.energy
...
Room.prototype.hubToFastFiller = function (hubLink, fastFillerLink) {
    ...
    hubLink.store.energy -=
        fastFillerLink.store.getCapacity(RESOURCE_ENERGY) -
        fastFillerLink.store.energy
}
...
Room.prototype.hubToController = function (hubLink, controllerLink) {
    ...
    if (
        ...
        controllerLink.store.getCapacity(RESOURCE_ENERGY) * 0.25
    )
        return
