...
Room.prototype.get = function (roomObjectName) {
    ...
    function findStructuresByType() {
        ...
        const structuresByType: Partial<
            Record<StructureConstant, Structure[]>
        > = {}
    ...
    function findCSitesByType() {
        ...
        const cSitesByType: Partial<
            Record<StructureConstant, ConstructionSite[]>
        > = {}
        ...
        for (const cSite of room.find(FIND_MY_CONSTRUCTION_SITES)) {
            ...
            if (!cSitesByType[cSite.structureType])
                cSitesByType[cSite.structureType] = []
    ...
    for (const structureType of constants.allStructureTypes) {
        ...
        new RoomObject({
            ...
            valueConstructor() {
                return (
                    room.roomObjects.structuresByType.getValue()[
                        structureType
                    ] || []
                )
            },
        ...
        new RoomObject({
            ...
            valueConstructor() {
                return (
                    room.roomObjects.cSitesByType.getValue()[structureType] ||
                    []
                )
            },
        })
     ...
     */
    function findClosestHarvestPos(
    ...
    new RoomObject({
        ...
        room,
        valueConstructor() {
            return findClosestHarvestPos(
                room.roomObjects.mineralHarvestPositions.getValue()
            )
        },
    })
    ...
    new RoomObject({
        ...
        valueConstructor() {
            return findClosestHarvestPos(
                room.roomObjects.source1HarvestPositions.getValue()
            )
        },
    })
    ...
    new RoomObject({
        ...
        valueConstructor() {
            return findClosestHarvestPos(
                room.roomObjects.source2HarvestPositions.getValue()
            )
        },
    })
    ...
    function findFastFillerPositions() {
        ...
        for (
            let index = fastFillerPositions.length - 1;
            index >= 0;
            index -= 1
        ) {
            ...
            const adjacentStructuresByType: Partial<
            ...
            for (const adjacentPosData of adjacentStructures) {
                ...
                if (!adjacentStructuresByType[structureType])
                    adjacentStructuresByType[structureType] = 0
            ...
            if (
                ...
                (adjacentStructuresByType[STRUCTURE_SPAWN] > 0 ||
                    adjacentStructuresByType[STRUCTURE_EXTENSION] > 1)
            )
                continue
    ...
    new RoomObject({
        ...
        valueConstructor() {
            return findSourceContainer(
                room.roomObjects.source1ClosestHarvestPos.getValue()
            )
        },
    })
    ...
    new RoomObject({
        ...
        valueConstructor() {
            return findSourceContainer(
                room.roomObjects.source2ClosestHarvestPos.getValue()
            )
        },
    })
    ...
    function findUsedSourceHarvestPositions() {
        ...
        const harvesterNames =
            room.memory.type === 'commune'
                ...
                : room.myCreeps.source1RemoteHarvester.concat(
                      room.myCreeps.source2RemoteHarvester
                  )
    ...
    function findUsedUpgradePositions() {
        ...
        const controllerContainer: StructureContainer =
            room.roomObjects.controllerContainer.getValue()
        ...
        const hubAnchor = unpackAsRoomPos(
            room.memory.stampAnchors.hub[0],
            room.name
        )
        ...
        const upgradePositions: RoomPosition[] =
            room.roomObjects.upgradePositions.getValue()
    ...
    function findControllerContainer() {
        ...
        const centerUpgradePos: RoomPosition =
            room.roomObjects.centerUpgradePos.getValue()
    ...
    function findMineralContainer() {
        ...
        const mineralHarvestPos: RoomPosition =
            room.roomObjects.closestMineralHarvestPos.getValue()
    ...
    function findFastFillerContainer(offset: number) {
        ...
        const structuresAsPos = room.lookForAt(
            LOOK_STRUCTURES,
            room.anchor.x + offset,
            room.anchor.y
        )
    ...
    function findLinkNearby(
        anchor: RoomPosition | undefined
    ): Id<Structure> | false {
        ...
        return links.find(
            link =>
                getRangeBetween(anchor.x, anchor.y, link.pos.x, link.pos.y) ===
                1
        )?.id
    }
    ...
    new RoomObject({
        ...
        valueConstructor() {
            return findLinkNearby(
                unpackAsRoomPos(room.memory.stampAnchors.hub[0], room.name)
            )
        },
    })
    ...
    function findStructuresForSpawning() {
        ...
        const spawnsAndExtensions: (StructureExtension | StructureSpawn)[] =
            room.roomObjects.spawn
                .getValue()
        ...
        const filteredSpawnStructures = spawnsAndExtensions.sort(
            (a, b) =>
                a.pos.getRangeTo(anchor.x, anchor.y) -
                b.pos.getRangeTo(anchor.x, anchor.y)
        )
    ...
    new RoomObject({
        ...
        valueConstructor() {
            ...
            const remotesWithEfficacies = room.memory.remotes.filter(function (
                roomName
            ) {
            ...
            return remotesWithEfficacies.sort(function (a1, b1) {
                return (
                    Memory.rooms[a1].sourceEfficacies.reduce(
                        (a2, b2) => a2 + b2
                    ) /
                        Memory.rooms[a1].sourceEfficacies.length -
                    Memory.rooms[b1].sourceEfficacies.reduce(
                        (a2, b2) => a2 + b2
                    ) /
...
Room.prototype.advancedFindPath = function (opts: PathOpts): RoomPosition[] {
    ...
    function generateRoute(): Route | undefined {
        ...
        const route = Game.map.findRoute(
            opts.origin.roomName,
            opts.goal.pos.roomName,
            {
            ...
            roomCallback(roomName) {
                ...
                if (opts.avoidStationaryPositions) {
                    ...
                    for (const sourceName of sources) {
                        ...
                        const harvestPositions: Pos[] = room.get(
                            `${sourceName}HarvestPositions`
                        )
                    ...
                    if (room.anchor) {
                        ...
                        const deliverUpgradePos = room.anchor.findClosestByPath(
                            upgradePositions,
                            {
                                ignoreCreeps: true,
                            }
                    ...
                    const fastFillerPositions: Pos[] = room.get(
                        'fastFillerPositions'
                    )
...
Room.prototype.findType = function (scoutingRoom: Room) {
    ...
    if (controller) {
        ...
        function isUnReservedRemote() {
            ...
            const creepsNotMine: Creep[] = room
                .get('enemyCreeps')
...
Room.prototype.makeRemote = function (scoutingRoom) {
    ...
    const distanceFromScoutingRoom = advancedFindDistance(
        scoutingRoom.name,
        room.name,
        {
            keeper: Infinity,
    ...
    if (distanceFromScoutingRoom < 4) {
        ...
        if (
            ...
            scoutingRoom.name === room.memory.commune
        )
            return true
        ...
        */
        ...
        if (
            room.memory.type !== 'remote' ||
            !Memory.communes.includes(room.memory.commune)
        ) {
            room.memory.type = 'remote'
        ...
        const currentAvgSourceEfficacy =
            room.memory.sourceEfficacies.reduce((sum, el) => sum + el) /
            room.memory.sourceEfficacies.length
        const newAvgSourceEfficacy =
            newSourceEfficacies.reduce((sum, el) => sum + el) /
            newSourceEfficacies.length
...
Room.prototype.findStoredResourceAmount = function (resourceType) {
    ...
    for (const storageStructure of storageStructures) {
        ...
        room.storedResources[resourceType] +=
            storageStructure.store.getUsedCapacity(resourceType)
    }
...
Room.prototype.findTasksOfTypes = function (createdTaskIDs, types) {
    ...
    for (const taskID in createdTaskIDs) {
        ...
        const task: RoomTask =
            room.global.tasksWithoutResponders[taskID] ||
            room.global.tasksWithResponders[taskID]
...
Room.prototype.distanceTransform = function (
    ...
    for (
        let x = Math.max(x1 - 1, 0);
        x <= Math.min(x2 + 1, constants.roomDimensions);
        x += 1
    ) {
        for (
            let y = Math.max(y1 - 1, 0);
            y <= Math.min(y2 + 1, constants.roomDimensions);
            y += 1
        ) {
        ...
        while (thisGeneration.length) {
            ...
            for (const pos of thisGeneration) {
                ...
                for (const adjacentPos of adjacentPositions) {
                    ...
                    if (
                        canUseWalls &&
                        opts.CM.get(adjacentPos.x, adjacentPos.y) !== 255
                    )
                        canUseWalls = false
...
Room.prototype.findCSiteTargetID = function (creep) {
    ...
    for (const structureType of constants.structureTypesByBuildPriority) {
        ...
        const cSitesOfType: ConstructionSite[] = room.get(
            `${structureType}CSite`
        )
        ...
        const anchor: RoomPosition =
            room.anchor || creep?.pos || new RoomPosition(25, 25, room.name)
...
Room.prototype.findRepairTargets = function (
    workPartCount,
    excludedIDs = new Set()
) {
    const room = this
...
Room.prototype.groupRampartPositions = function (
    rampartPositions,
    rampartPlans
) {
    const room = this
    ...
    for (const packedPos of rampartPositions) {
        ...
        groupedPositions[groupIndex] = [
            new RoomPosition(pos.x, pos.y, room.name),
        ]
...
Room.prototype.advancedConstructStructurePlans = function () {
    ...
    for (const stampType in stamps) {
        ...
        for (const packedStampAnchor of room.memory.stampAnchors[
            stampType as StampTypes
        ]) {
            ...
            for (const structureType in stamp.structures) {
                ...
                if (
                    room.get(structureType as BuildableStructureConstant)
                        .length +
                        room.get(
                            `${
                ...
                if (
                    ...
                    (!room.storage || room.controller.level < 4)
                )
                    continue
    ...
    if (room.energyCapacityAvailable >= 800) {
        ...
        function planPos(x: number, y: number) {
            ...
            const structureType =
                constants.numbersByStructureTypes[structurePlans.get(x, y)]
...
Room.prototype.findRoomPositionsInsideRect = function (x1, y1, x2, y2) {
    ...
    for (let x = x1; x <= x2; x += 1) {
        for (let y = y1; y <= y2; y += 1) {
            ...
            if (
                x < 0 ||
                x >= constants.roomDimensions ||
                y < 0 ||
                y >= constants.roomDimensions
            )
                continue
...
Room.prototype.createClaimRequest = function () {
    ...
    const closestCommuneRange = Game.map.getRoomLinearDistance(
        closestClaimTypeName,
        this.name
    )
