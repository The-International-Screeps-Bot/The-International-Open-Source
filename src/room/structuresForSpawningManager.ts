...
export function structuresForSpawningManager(room: Room) {
    ...
    let structuresForSpawning: (StructureSpawn | StructureExtension)[] =
        room.get('structuresForSpawning')
    ...
    for (const sourceName of sourceNames) {
        ...
        structuresForSpawning = structuresForSpawning.filter(
            structure =>
                getRange(
                    structure.pos.x - closestHarvestPos.x,
                    structure.pos.y - closestHarvestPos.y
                ) > 1
        )
    }
    ...
    if (
        ...
        structuresForSpawning = structuresForSpawning.filter(
            structure =>
                getRange(
                    structure.pos.x - room.anchor.x,
                    structure.pos.y - room.anchor.y
                ) > 2
        )
    }
