...
Room.prototype.advancedSell = function (resourceType, amount) {
    ...
    const mySpecificOrders =
        internationalManager.myOrders[this.name]?.[ORDER_SELL][resourceType] ||
        []
    ...
    for (const order of internationalManager.getBuyOrders(resourceType)) {
        ...
        Game.market.deal(
            order.id,
            Math.min(amount, order.remainingAmount),
            this.name
        )
        return true
    }
...
Room.prototype.advancedBuy = function (resourceType, amount) {
    ...
    const mySpecificOrders =
        internationalManager.myOrders[this.name]?.[ORDER_BUY][resourceType] ||
        []
