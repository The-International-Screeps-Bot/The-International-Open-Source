...
RemoteDefender.prototype.advancedHeal = function () {
    ...
    let top = Math.max(
        Math.min(creep.pos.y - 1, constants.roomDimensions - 2),
        2
    )
    let left = Math.max(
        Math.min(creep.pos.x - 1, constants.roomDimensions - 2),
        2
    )
    let bottom = Math.max(
        Math.min(creep.pos.y + 1, constants.roomDimensions - 2),
    ...
    for (const posData of adjacentCreeps) {
        ...
        if (!posData.creep.my && !allyList.has(posData.creep.owner.username))
            continue
    ...
    (top = Math.max(
        Math.min(creep.pos.y - 3, constants.roomDimensions - 2),
        2
    )),
        (left = Math.max(
            Math.min(creep.pos.x - 3, constants.roomDimensions - 2),
            2
        )),
        (bottom = Math.max(
            Math.min(creep.pos.y + 3, constants.roomDimensions - 2),
    ...
    const nearbyCreeps = room.lookForAtArea(
        LOOK_CREEPS,
        top,
        left,
        bottom,
...
RemoteDefender.prototype.advancedAttackAttackers = function () {
    ...
    const enemyAttackers = room.enemyCreeps.filter(enemyCreep =>
        /* !enemyCreep.isOnExit() && */ enemyCreep.hasPartsOfTypes([
            ATTACK,
            RANGED_ATTACK,
        ])
    )
    ...
    if (!enemyAttackers.length) {
        ...
        const range = getRange(
            creep.pos.x - enemyCreep.pos.x,
            creep.pos.y - enemyCreep.pos.y
        )
