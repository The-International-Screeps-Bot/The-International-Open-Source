export function rampartPlanner(room: Room) {
    ...
    outer: do {
        dirty = false;
        let gen = 0,
            igen = {}; // Inverse Generationalize: igen[i] is the flood-fill generation of tile i.
        ...
        while (igen_field.length && igen_open.length) {
            for (const o of igen_open) { igen[o] = gen; }
            gen++;
        ...
        for (const source of gen0) { // "Source" is the mincut term for the exits of the room.
            ...
            while (path.length) { // DFS all possible paths
                ...
                if (!child) { deadend.add(head);
                    path.pop(); continue; } // Hit leaf node in DFS.
