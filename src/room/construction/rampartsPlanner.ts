...
let parents = {}, dirty = false;
outer: do {
    dirty = false;
    let gen = 0, igen = {}; // Inverse Generationalize: igen[i] is the flood-fill generation of tile i.
    let igen_all = _.filter(allnodes, x=>!parents[x]);
    ...
    while (igen_field.length && igen_open.length) {
        for (const o of igen_open) { igen[o]=gen; } gen++;
    ...
    for (const source of gen0) { // "Source" is the mincut term for the exits of the room.
        ...
        while (path.length) { // DFS all possible paths
            ...
            if (!child) { deadend.add(head); path.pop(); continue; } // Hit leaf node in DFS.
            if (sinks.includes(child)) { // Found a "sink".  Log a maxflow path.
                if (path.length == 1) { break; }
                for (let i=1; i<path.length; i++) { parents[path[i]]=path[i-1]; }
                dirty = true; continue outer;
            }
            path.push(child);
        }
    }
...
let open = gen0, closed = [];
while (open.length) { // Pseudo-Flood Fill, but "swept downstream" by maxflow path to parents.
    closed = _.union(open, closed);
    open = _(open).map(x=>getNeighbors(parents[x]||x)).flatten().uniq().filter(isntWall).difference(closed).difference(sinks).run();
}
