...
export function constructionManager(room: Room) {
    ...
    function manageControllerStructures() {
        ...
        const centerUpgradePos: RoomPosition | undefined =
            room.get('centerUpgradePos')
        if (!centerUpgradePos) return

        if (room.controller.level >= 5) {
            const controllerContainer: StructureContainer | undefined =
                room.get('controllerContainer')
