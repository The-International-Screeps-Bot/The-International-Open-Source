...
export class TerminalManager {
  ...
  doTransfers(): boolean {
    ...
    if (amountOfEnergy > 450000) {
      let result = _.min(
        ...
        room =>
          (room.terminal.store[RESOURCE_ENERGY] || 0) + (room.storage.store[RESOURCE_ENERGY] || 0),
      ...
      for (let resource of [
        ...
        for (let comp in COMMODITIES[resource].components) {
          ...
          if (comp == RESOURCE_ENERGY) {
            buildCost +=
              this.room.memory[RoomMemoryKeys.marketData].aquire[RESOURCE_ENERGY] * amount
          } else {
            buildCost +=
              this.room.memory[RoomMemoryKeys.marketData].sellAvg[
                comp as
  ...
  extendSellOrders(
    ...
    for (let thisTarget of sellTarget) {
      ...
      let amountToAdjust =
        Math.min(targetOrderSize, totalBetweenStorageAndTerminal) - sellOrder.remainingAmount
      if (amountToAdjust > 0) {
  ...
  extendBuyOrders(
    ...
    for (let thisTarget of purchaseTarget) {
      ...
      let result = _.min(this.dataCache[ORDER_BUY][thisTarget.resource], ord => -ord.price) as
        | Order
        | number
