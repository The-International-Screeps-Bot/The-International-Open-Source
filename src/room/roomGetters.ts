...
Object.defineProperties(Room.prototype, {
    ...
    anchor: {
        get() {
            ...
            return (this._anchor =
                this.memory.stampAnchors &&
                this.memory.stampAnchors.fastFiller.length
                    ? unpackAsRoomPos(
                          this.memory.stampAnchors.fastFiller[0],
                          this.name
                      )
                    : undefined)
        },
    ...
    cSiteTarget: {
        get() {
            ...
            for (const structureType of constants.structureTypesByBuildPriority) {
                ...
                const anchor =
                    this.anchor || new RoomPosition(25, 25, this.name)

                return (this.memory.cSiteTargetID =
                    anchor.findClosestByRange(cSitesOfType).id)
            }
    ...
    spawningStructures: {
        get() {
            ...
            return (this._spawningStructures = this.get('spawn').concat(
                this.get('extension')
            ))
        },
    },
    taskNeedingSpawningStructures: {
        get() {
            if (this._taskNeedingSpawningStructures)
            ...
            for (const pos of this.global.stampAnchors.extensions) {
                ...
                for (const structure of structuresAtPos) {
                    ...
                    this._taskNeedingSpawningStructures.push(
                        structure as StructureSpawn | StructureExtension
                    )
                    break
                }
            }
    ...
    spawningStructuresByPriority: {
        get() {
            ...
            for (const sourceName of sourceNames) {
                ...
                const closestHarvestPos: RoomPosition | undefined = this.get(
            ...
            return this._spawningStructuresByPriority.concat(
                this.taskNeedingSpawningStructures.sort(
                    (a, b) =>
                        getRange(
                            a.pos.x - this.anchor.x,
                            a.pos.y - this.anchor.y
                        ) -
                        getRange(
                            b.pos.x - this.anchor.x,
                            b.pos.y - this.anchor.y
                        )
                )
            )
        },
    ...
    sourceHarvestPositions: {
        get() {
            if (this.global.sourceHarvestPositions)
                return this.global.sourceHarvestPositions
