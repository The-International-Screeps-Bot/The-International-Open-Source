...
module.exports = {
    run: function taskManger(room, haulers) {
        ...
        if (findHaulersOfRoom().length > 0) {
            ...
            if (controllerLink == null && controllerContainer != null && storage && storage.store[RESOURCE_ENERGY] >= 30000 && controllerContainer.store[RESOURCE_ENERGY] <= findHaulersOfRoom()[0].store.getCapacity()) {
            ...
            let tombstones = room.find(FIND_TOMBSTONES, {
                filter: (s) => s.store[RESOURCE_ENERGY] >= findHaulersOfRoom()[0].store.getCapacity() * 0.5
            })

            if (tombstones.length > 0) {

                if (!findCreepWithTask("tombstone", 1) && findCreepWithoutTask()) {

                    findCreepWithoutTask().memory.task = "tombstone"
                }
            }
