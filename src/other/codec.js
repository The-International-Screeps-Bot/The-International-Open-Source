// eslint-disable
import { buildableStructureTypes, packedPosLength, stampKeys } from 'international/constants';
 ...
 */
export function unpackId(packedId) {
    let id = '';
    let current;
    for (let i = 0; i < 6; ++i) {
 ...
 */
export function packIdList(ids) {
    let str = '';
 ...
 */
export function unpackIdList(packedIds) {
    const ids = [];
 ...
 */
export function packXYAsCoord(x, y) {
 ...
 */
export function unpackCoord(char) {
 ...
 */
export function unpackCoordAsPos(packedCoord, roomName) {
 ...
 */
export function reversePosList(coordList) {
    return coordList
        .match(/.{1,3}/g)
        .reverse()
        .join('');
 ...
 * extra commas and "" when memory gets stringified.
 */
export function packCoordList(coords) {
    let str = '';
 ...
 */
export function unpackCoordList(chars) {
    const coords = [];
 ...
 */
export function unpackCoordListAsPosList(packedCoords, roomName) {
    const positions = [];
    let coord;
 ...
 * Unpack room name
 */
export function unpackRoomName(q, x, y) {
    let roomName;
    switch (q) {
        case 0:
            roomName = `W${x}N${y}`;
 ...
 * Packs a RoomPosition as a 3 Uint8 characters.
 */
export function packXYAsPos(x, y, roomName) {
 ...
 */
export function unpackPos(chars) {
 ...
 */
export function packPosList(posList) {
    let str = '';
 ...
 */
export function unpackPosList(chars) {
    const posList = [];
 ...
 */
export function unpackPosAt(chars, index = 0) {
    if (chars.length < packedPosLength * (index + 1))
        return undefined;
 ...
 */
export function packBasePlanCoord(planCoords) {
    let packedCoords = '';
 ...
 */
export function unpackBasePlanCoords(packedPlanCoords) {
    const planCoords = [];
    for (const packedPlanCoord of packedPlanCoords.split(',')) {
        if (!packedPlanCoord.length)
            continue;
 ...
 * Pack a planned cord for base building
 */
export function packRampartPlanCoord(planCoord) {
    return encode(new Uint8Array(Object.values(planCoord)));
}
/**
 * Unpack a planned cord for base building
 */
export function unpackRampartPlanCoord(chars) {
...
export function packStampAnchors(stampAnchors) {
    const packedStampAnchors = {};
...
export function unpackStampAnchors(packedStampAnchors) {
    const stampAnchors = {};
