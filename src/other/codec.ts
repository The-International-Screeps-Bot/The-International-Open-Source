...
export function packCoord(coord: Coord) {
    ...
    const packedCoord = encode(new Uint8Array([coord.x, coord.y]))
    return packedCoord
}
...
export function packXYAsCoord(x: number, y: number) {
    const packedCoord = encode(new Uint8Array([x, y]))
    return packedCoord
}
...
export function unpackCoord(char: string) {
    ...
    const coord = { x: decoded[0], y: decoded[1] }
...
export function packPos(pos: RoomPosition) {
    ...
    const packedPos = encode(new Uint8Array([map.quadrant, map.x, map.y, pos.x, pos.y]))
...
export function packXYAsPos(x: number, y: number, roomName: string) {
    ...
    const packedPos = encode(new Uint8Array([map.quadrant, map.x, map.y, x, y]))
...
export function unpackPos(chars: string) {
    ...
    return new RoomPosition(coord.x, coord.y, coord.roomName)
}
...
export function packRampartPlanCoord(planCoord: RampartPlanCoord) {
    ...
    const packedCoord = encode(new Uint8Array(Object.values(planCoord)))
    return packedCoord
}
