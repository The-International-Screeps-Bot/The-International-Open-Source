 ...
 */
export function findObjectWithID<T extends Id<any>>(
    ID: T
): fromId<T> | undefined {
 ...
 */
export function findPositionsInsideRect(
    x1: number,
    y1: number,
    x2: number,
    y2: number
...
export function findCreepInQueueMatchingRequest(
    queue: string[],
    requestPackedPos: number
) {
...
export function findRemoteSourcesByEfficacy(
    roomName: string
): ('source1' | 'source2')[] {
...
export function findLargestTransactionAmount(
    ...
    while (
        Game.market.calcTransactionCost(amount, roomName1, roomName2) > budget
    ) {
...
export function findClosestCommuneName(roomName: string) {
    const communesNotThis = Memory.communes.filter(
        communeName => roomName !== communeName
    )
