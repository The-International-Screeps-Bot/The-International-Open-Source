...
InternationalManager.prototype.tickConfig = function () {
    ...
    Memory.stats = {
        ...
        totalGCL:
            Math.pow(Game.gcl.level - 1, 2.4) * 1000000 + Game.gcl.progress,
    ...
    for (const roomName in Game.rooms) {
        ...
        for (
            let index = room.memory.remotes.length - 1;
            index >= 0;
            index -= 1
        ) {
            ...
            if (
                roomMemory.type !== 'remote' ||
                roomMemory.commune !== room.name
            ) {
            ...
            if (
                ...
                remote.controller.reservation.ticksToEnd >=
                    roomMemory.sourceEfficacies.reduce((a, b) => a + b) * 2
            ) {
            ...
            roomMemory.needs[remoteNeedsIndex.source2RemoteHarvester] =
                roomMemory.source2 ? 3 : 0
        ...
        if (
            room.memory.claimRequest &&
            !Memory.claimRequests[room.memory.claimRequest]
        )
