    ...
    for (const roomName of Memory.communes) {
        ...
        for (const remoteName of room.memory.remotes) {
            ...
            if (isReserved) {
                ...
                remoteMemory.needs[remoteNeedsIndex.source2RemoteHarvester] +=
                    remoteMemory.source2 ? 3 : 0
            }
            ...
            if (remote) {
                ...
                for (const enemyCreep of remote.enemyCreeps) {
                    ...
                    remoteMemory.needs[remoteNeedsIndex.remoteDefender] +=
                        enemyCreep.findStrength()
                }
            ...
            for (
                let index = 0;
                index < remoteMemory.sourceEfficacies.length;
                index += 1
            ) {
                ...
                remoteMemory.needs[remoteNeedsIndex.remoteHauler] +=
                    findCarryPartsRequired(efficacy, income)
            }
        }
    }
}
