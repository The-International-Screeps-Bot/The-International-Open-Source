import { getAvgPrice, packXYAsNum, randomTick, roundTo, } from './utils';
import { WorkRequestKeys, mmoShardNames, roomDimensions, RoomMemoryKeys, minerals, } from './constants';
/**
...
export class CollectiveManager {
    ...
    newCustomCreepID() {
        ...
        for (; this.customCreepIDIndex < this.customCreepIDs.length; this.customCreepIDIndex++) {
            if (this.customCreepIDs[this.customCreepIDIndex])
                continue;
 ...
 */
    ...
    get minCredits() {
        if (this._minCredits !== undefined)
            return this._minCredits;
    ...
    get workRequestsByScore() {
        if (this._workRequestsByScore)
            return this._workRequestsByScore;
        return (this._workRequestsByScore = Object.keys(Memory.workRequests).sort((a, b) => {
            var _a, _b;
            return ((_a = Memory.workRequests[a][WorkRequestKeys.priority]) !== null && _a !== void 0 ? _a : Memory.rooms[a][RoomMemoryKeys.score] +
                Memory.rooms[a][RoomMemoryKeys.dynamicScore]) -
                ((_b = Memory.workRequests[b][WorkRequestKeys.priority]) !== null && _b !== void 0 ? _b : Memory.rooms[b][RoomMemoryKeys.score] +
    ...
    get avgCommunesPerMineral() {
        ...
        for (const mineralType in this.mineralCommunes) {
            sum += this.mineralCommunes[mineralType];
        }
     ...
     */
    get funnelOrder() {
        if (this._funnelOrder)
            return this._funnelOrder;
        let funnelOrder = [];
        // organize RCLs 1-7
        const communesByLevel = {};
        for (let i = 6; i < 8; i++)
            communesByLevel[i] = [];
        for (const roomName of global.communes) {
            ...
            if (!communesByLevel[controller.level])
                continue;
    ...
    get maxCSitesPerRoom() {
        if (this._maxCSitesPerRoom)
            return this._maxCSitesPerRoom;
